...
def _run_package_test(runner, test_config):
    ...
    if 'installed' in test_config:
        ...
        if package_info['installed'] != expected_installed:
            ...
            if expected_installed:
                result['details']['error'] = f"Package '{package_name}' is not installed"
            else:
                result['details']['error'] = f"Package '{package_name}' is installed but should not be"
    ...
    if 'version' in test_config and package_info['installed']:
        ...
        if package_info['version'] != expected_version:
            result['result'] = False
            result['details']['error'] = f"Expected version '{expected_version}', found '{package_info['version']}'"
