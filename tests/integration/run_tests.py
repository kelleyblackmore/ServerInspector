...
SSH_CONFIG = {"username": "testuser", "password": "testpass", "hostname": "localhost"}
...
class TestRunner:
    ...
    def check_docker_environment(self) -> bool:
        ...
        try:
            ...
            console.print(
                f"[green]✓ {running_count}/{len(DISTRIBUTIONS)} containers running[/green]"
            )
    ...
    def run_tests_for_distribution(self, dist_key: str) -> Dict[str, Any]:
        ...
        for config_file in dist_info["configs"]:
            ...
            if result and "error" not in result:
                ...
                all_tests.append(
                    {"name": config_file, "status": "error", "error": error_msg}
                )
                total_failed += 1
        ...
        console.print(
            f"  [bold]Results: {total_passed} passed, {total_failed} failed[/bold]"
        )
    ...
    def run_all_tests(self, distributions: Optional[List[str]] = None):
        ...
        for dist_key in distributions:
            if dist_key not in DISTRIBUTIONS:
                console.print(
                    f"[yellow]Skipping unknown distribution: {dist_key}[/yellow]"
                )
                continue
...
def main():
    ...
    if args.list:
        ...
        for key, info in DISTRIBUTIONS.items():
            console.print(
                f"  [cyan]{key:12}[/cyan] - {info['name']:25} "
