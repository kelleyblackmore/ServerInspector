...
def test_service_status():
    ...
    for service_name in test_services:
        ...
        basic.add(
            f"Exists: {'✓' if status_dict['exists'] else '✗'} {status_dict['exists']}"
        )
        basic.add(
            f"Running: {'✓' if status_dict['running'] else '✗'} {status_dict['running']}"
        )
        basic.add(
            f"Enabled: {'✓' if status_dict['enabled'] else '✗'} {status_dict['enabled']}"
        )
        ...
        if status_dict.get("dependencies"):
            deps = tree.add(
                f"[yellow]Dependencies ({len(status_dict['dependencies'])})[/yellow]"
            )
