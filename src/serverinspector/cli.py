"""
Command-line interface for serverinspector.
"""
...
def cli(debug):
    """serverinspector - Test and audit server configurations."""
    if debug:
...
@click.option(
    "--password-stdin",
    envvar="serverinspector_PASSWORD",
...
def run(
    ...
    try:
        ...
        # Create the serverinspector instance
...
@cli.command()
def init():
    """Initialize a new serverinspector test file."""
    try:
        example_test = """---
# serverinspector Test Configuration

# General information
title: Basic Server Test
description: Tests basic server functionality and configuration
author: serverinspector User
...
"""
        ...
        output_file = "serverinspector-test.yaml"
...
def check(check_type, target, **options):
    ...
    try:
        ...
        try:
            ...
            logger.error("The 'serverinspector.checkers' module is not available.")
            console.print(
                "[bold red]Error:[/bold red] The checkers module is not installed or not in the Python path."
            )
            console.print(
                "Make sure you've installed serverinspector correctly and the src directory is in your Python path."
            )
            sys.exit(1)
