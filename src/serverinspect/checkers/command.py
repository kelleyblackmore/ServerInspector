...
def check(params):
    ...
    try:
        ...
        result['details']['stdout'] = stdout[:200] + ('...' if len(stdout) > 200 else '')
        result['details']['stderr'] = stderr[:200] + ('...' if len(stderr) > 200 else '')
        ...
        if 'exit_code' in params:
            ...
            if exit_code != expected_exit_code:
                result['message'] = f"Command exited with code {exit_code}, expected {expected_exit_code}"
                return result
        ...
        if 'stdout_contains' in params:
            ...
            if expected_stdout not in stdout:
                result['message'] = f"Command stdout does not contain '{expected_stdout}'"
                return result
        ...
        if 'stderr_contains' in params:
            ...
            if expected_stderr not in stderr:
                result['message'] = f"Command stderr does not contain '{expected_stderr}'"
                return result
