...
def check(params):
    ...
    try:
        ...
        if running:
            ...
            process_count = (
                int(count_proc.stdout.strip()) if count_proc.returncode == 0 else 0
            )
        else:
            process_count = 0
        ...
        if "running" in params:
            ...
            if running == expected_running:
                ...
                if expected_running:
                    result["message"] = (
                        f"Process '{process_name}' is running as expected"
                    )
                else:
                    result["message"] = (
                        f"Process '{process_name}' is not running as expected"
                    )
            else:
                if expected_running:
