...
@click.option('--exists/--not-exists', default=None, help='For file/package checks: whether the file/package should exist')
@click.option('--contains', help='For file/command checks: text that should be contained in the output')
@click.option('--exit-code', type=int, help='For command checks: expected exit code')
@click.option('--running/--not-running', default=None, help='For service/process checks: whether it should be running')
@click.option('--listening/--not-listening', default=None, help='For port checks: whether the port should be listening')
@click.option('--protocol', type=click.Choice(['tcp', 'udp']), default='tcp', help='For port checks: protocol to check')
def check(check_type, target, **options):
    ...
    try:
        ...
        try:
            ...
            console.print("[bold red]Error:[/bold red] The checkers module is not installed or not in the Python path.")
            console.print("Make sure you've installed ServerInspect correctly and the src directory is in your Python path.")
            sys.exit(1)
        ...
        try:
            ...
            console.print(f"[bold red]Error:[/bold red] Could not import the needed module: {str(e)}")
            console.print("This might be due to incomplete installation or Python path issues.")
            sys.exit(1)
