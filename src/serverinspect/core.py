...
class ServerInspect:
    ...
    def __init__(self, config=None, host=None, username=None, 
        ...
        logger.debug(f"Initialized ServerInspect with runner: {self.runner.__class__.__name__}")
    ...
    def run_tests(self):
        ...
        for test_config in self.config['tests']:
            try:
                ...
                logger.error(f"Error running test {test_config.get('name', 'unknown')}: {str(e)}")
                test_result = {
        ...
        logger.info(f"Tests completed: {results['summary']['passed']} passed, "
