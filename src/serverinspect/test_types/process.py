...
def _run_process_test(runner, test_config):
    ...
    if "min_count" in test_config:
        ...
        if process_info["count"] < min_count:
            ...
            result["details"][
                "error"
            ] = f"Expected at least {min_count} instance(s) of '{process_name}', found {process_info['count']}"
    ...
    if "max_count" in test_config:
        ...
        if process_info["count"] > max_count:
            ...
            result["details"][
                "error"
            ] = f"Expected at most {max_count} instance(s) of '{process_name}', found {process_info['count']}"
    ...
    if "user" in test_config and process_info["running"]:
        ...
        if actual_user != expected_user:
            ...
            result["details"][
                "error"
            ] = f"Process '{process_name}' is running as '{actual_user}', expected '{expected_user}'"
