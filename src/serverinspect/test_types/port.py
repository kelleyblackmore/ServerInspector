...
def _run_port_test(runner, test_config):
    ...
    if 'listening' in test_config:
        ...
        if port_listening != expected_listening:
            ...
            if expected_listening:
                ...
                result['details']['error'] = f"Port {port}/{protocol} is listening but should not be"
    ...
    if 'process' in test_config and port_listening:
        ...
        result['details']['actual_process'] = actual_process if actual_process else "None"
        
        if expected_process not in actual_process:
            result['result'] = False
            result['details']['error'] = f"Port {port}/{protocol} is used by '{actual_process}' instead of '{expected_process}'"
